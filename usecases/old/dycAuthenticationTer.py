...
def virtual_network():
    ...
    topologie.addLink(("AS",1),("fabric",3))
    
    return topologie

#Policies
def IO_policy(VID):
    i1 = match(edge=VID, src="guests", dst="WebServer") >> tag("non_auth_flows") >> forward("fabric")
...
def fabric_policy(VID):
    ...
    t4 = catch(fabric=VID, src="IO", flow="non_auth_flows") >> carry(dst="AS")
    return t1 + t2 + t3 + t4

#Main function
def main():
    in_network_functions = IO_policy("IO") + AC_policy("AC") + AS_policy("AS")
