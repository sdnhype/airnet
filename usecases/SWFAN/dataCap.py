...
#Policies
def IO_policy(VID):
    i1 = match(edge=VID, dst="server1") >> tag("in_web_flows") >> forward("fabric")
    #i3 = match(edge=VID, dst="server2") >> tag("in_web_flows") >> forward("fabric")
    i2 = match(edge=VID, dst="client1")  >> (checkDataCap(id="fct1") + forward("client1"))
    #i4 = match(edge=VID, dst="client2")  >> (checkDataCap(id="fct2") + forward("client2"))
    return i1 + i2 #+ i3 + i4

def AC_policy(VID):
    i1 = match(edge=VID, dst="server1") >> forward("server1")
    #i3 = match(edge=VID, dst="server2") >> forward("server2")
    i2 = match(edge=VID, src="server1") >> tag("out_web_flows") >> forward("fabric")
